...
namespace Lisbeth.API.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class SpentTimeTypeController : ControllerBase
    {
        private readonly ILogger<SpentTimeTypeController> _logger;
        private readonly ISpentTimeTypeService _service;
        private readonly IMapper _mapper;
        private readonly IUriService _uri;

        public SpentTimeTypeController(ILogger<SpentTimeTypeController> logger, ISpentTimeTypeService service, IMapper mapper, IUriService uri)
        {
            _logger = logger;
            _service = service;
            _mapper = mapper;
        ...
        {
            var res = await _service.GetAsync<SpentTimeTypeResDto>(id);

            return res is null
                ? NotFound()
                : Ok(new Response<SpentTimeTypeResDto>(res));
        }
        ...
        {
            var spec = new Specifications<SpentTimeType>(x => x.Name == name);
            var res = await _service.GetBySpecificationsAsync<SpentTimeTypeResDto>(filter, spec);
            if (res is null)
                return NotFound();
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] SpentTimeTypeReqDto dto)
        {
        ...
        [HttpPut]
        public async Task<IActionResult> Put([FromBody] SpentTimeTypeReqDto dto)
        {
